...
int main(int argc, char *argv[]){
     ...
     * Create the segment.
     */
    if ((shmid = shmget(key, (NUM_OF_RANDOMS+3) * sizeof(int), IPC_CREAT | 0666)) < 0) {
        perror("shmget");
        exit(1);
    }
	...
	s[2] = 0; //Will indicate when third client is finished
	s = s+3;
	getRandoms(s);
	...
	/* 
	 * Sort the last forth of the array.
	 */
	s = shm + 3;
	int limit = NUM_OF_RANDOMS;
	while(limit > NUM_OF_RANDOMS/4 - 1){
		int j;
		for (j = NUM_OF_RANDOMS/4; j < limit; j++){
			int temp;
			if(s[j] > s[j+1]){
				temp = s[j+1];
				s[j+1] = s[j];
	...
	int i1 = 0; 					//index
	int i2 = NUM_OF_RANDOMS/4;
	int i3 = 2*NUM_OF_RANDOMS/4;
	int i4 = 3*NUM_OF_RANDOMS/4;
	
	int l1 = NUM_OF_RANDOMS/4; 		//limit
	int l2 = 2*NUM_OF_RANDOMS/4;
	int l3 = 3*NUM_OF_RANDOMS/4;
	int l4 = NUM_OF_RANDOMS;
	int placeInFinal = 0;
	s = shm + 3;
	while(placeInFinal < NUM_OF_RANDOMS){
		if(s[i1] < s[i2] && s[i1] < s[i3] && s[i1] < s[i4]){
		...
		}else if(s[i2] < s[i1] && s[i2] < s[i3] && s[i2] < s[i4]){
		...
		}else if(s[i3] < s[i1] && s[i3] < s[i2] && s[i3] < s[i4]){
			...
			finalNumberArray[placeInFinal] = s[i4];
			if(i4 < l4){
				i4++;
			}else{
				s[i4] = 9999999;
