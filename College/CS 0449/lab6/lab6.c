...
void heapWalk( char * caption)
{
	...
	do
	{
		...
		int blockheader = blockstart[0] + blockstart[1] + blockstart[2] + blockstart[3];
		...
		int blocksize = blockheader & 4294967294;
		baseoffset = (int)blockstart - (int)base;
		int payload = blocksize - 2*sizeof(int);
		printf("    %d  %s   %d\t%d\t  %d bytes   %d bytes\t   %d\n",blockNum, statusString, (int)base + baseoffset, baseoffset, payload, 2*sizeof(int), blocksize);
