...
const upload = multer({
    ...
    fileFilter(req, file, cb) {
        if (!file.originalname.match(/\.(jpg|jpeg|png)$/)) return cb(new Error('Please upload a PDF'));
...
router.post('/users/me/avatar', auth, upload.single('avatar'), async (req, res) => {
    ...
    req.user.avatar = await buffer.cover(250, 250).getBufferAsync(jimp.MIME_PNG);
    await req.user.save();
    res.send();
}, (error, req, res, next) => {
